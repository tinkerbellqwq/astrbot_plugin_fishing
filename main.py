...
class FishingPlugin(Star):
    ...
    async def register_user(self, event: AstrMessageEvent):
        ...
        # 如果用户昵称为空，则使用用户ID
        result = self.FishingService.register(user_id, event.get_sender_name() if event.get_sender_name() else str(user_id))
    ...
    async def show_ranking(self, event: AstrMessageEvent):
        ...
        try:
            ...
            message += "\n🎣 钓鱼大师榜 🎣\n"
            for idx, user in enumerate(sorted(top_users, key=lambda x: x.get('total_fishing_count', 0), reverse=True)[:10], 1):
                rank_emoji = "🥇" if idx == 1 else "🥈" if idx == 2 else "🥉" if idx == 3 else f"{idx}."
                message += f"{rank_emoji} {user.get('nickname', '未知用户')} - {user.get('total_fishing_count', 0)}条鱼\n"
                
            message += "\n⚖️ 总重量榜 ⚖️\n"
            for idx, user in enumerate(sorted(top_users, key=lambda x: x.get('total_weight_caught', 0), reverse=True)[:10], 1):
                rank_emoji = "🥇" if idx == 1 else "🥈" if idx == 2 else "🥉" if idx == 3 else f"{idx}."
